...
// 教学注释: v6 是相对 v5 的第一次模块化流水线实现。
// 这里把原先集中写在一个文件里的取指/译码/执行/访存/回写拆成 IFU/IDU/EXU/LSU/WBU 五段。
// 观察信号命名时可把 `IFU_* -> IDU_* -> EXU_* -> LSU_* -> WBU_*` 看成级间流动的数据与控制信息。
// 本版已经引入 dnpc/flush/CSR 接口，但 hazard 处理仍较简化，主要为 v7 的真实前递与停顿机制预留接口。
module myCPU (
    input cpu_clk,
    input cpu_rst,
